...
export const ExpensesDetails = ({ choices, inputDate, inputDateRecordList, theme }: Props) => {
  ...
  while (date.getMonth() === inputDate.getMonth()) {
    (inputDateRecordList.forEach(item => {
      if (item.date.getDate() === date.getDate() &&
        item.category !== CardCategory.INCOME
      ) {
        if (inputMonth.length > 0) {
          if (inputMonth[inputMonth.length - 1].getDate() !== date.getDate()) { inputMonth.push(new Date(date)) }
        } else if (inputMonth.length === 0) { inputMonth.push(new Date(date)) }
      }
    }))
      ...
      .map(choice => {
        return {
          ...
          data: inputMonth.map((day) => {
            const incomeData = inputDateRecordList.filter(i => i.category === choice)
            let output: number = 0
            incomeData.forEach(item => {
              if (item.date.getDate() === day.getDate()) { output += item.amount }
            })
            return output
          }),
